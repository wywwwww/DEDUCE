...
class Buffer:
    ...
    def greedy_balancing_sampler(self, examples, labels=None, logits=None):
        """
        Implements the Greedy Balancing Sampler for updating the buffer.
        :param examples: a batch of examples (image tensors)
        :param labels: the corresponding labels (tensor)
        """
        for i in range(len(labels)):
            ...
            # Calculate the target count per class
            num_classes = len(self.class_counts)
            target_per_class = self.buffer_size // max(1, num_classes)

            # Convert label to scalar
            label_scalar = label.item()

            # Check if the class needs balancing
            if label_scalar not in self.class_counts or self.class_counts[label_scalar] < target_per_class:
                if self.num_seen_examples < self.buffer_size:
                    # Add directly if buffer is not full
                    if not hasattr(self, 'examples'):
                        self.init_tensors(example.unsqueeze(0), label.unsqueeze(0), logit.unsqueeze(0), None)
                    self.examples[self.num_seen_examples] = example.to(self.device)
                    self.labels[self.num_seen_examples] = label.to(self.device)
                    self.logits[self.num_seen_examples] = logit.to(self.device)
                    self.class_counts[label_scalar] = self.class_counts.get(label_scalar, 0) + 1
                    self.num_seen_examples += 1
                else:
                    # Replace a sample from the largest class if buffer is full
                    largest_class = max(self.class_counts, key=self.class_counts.get)
                    indices = [idx for idx, lbl in enumerate(self.labels[:self.num_seen_examples]) if
                               lbl.item() == largest_class]
                    replace_index = random.choice(indices)
                    self.examples[replace_index] = example.to(self.device)
                    self.labels[replace_index] = label.to(self.device)
                    self.logits[replace_index] = logit.to(self.device)
